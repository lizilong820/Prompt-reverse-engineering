...
from app.video_sources.downloader import _open, download_video
...
def validate_platform_url(url: str, deadline: float | None = None) -> str:
    ...
    if platform is None:
        raise InvalidUploadError("仅支持抖音、快手、小红书、B站、YouTube 页面或公开视频直链")
    parsed = urlsplit(extract_url(url))
    try:
...
def _platform_error_message(platform: str, raw_message: str) -> str:
    ...
    if platform == "kuaishou":
        return "该快手公开页面暂时无法解析，请确认分享链接有效"
    if platform == "xiaohongshu":
        return "该小红书公开页面暂时无法解析，请确认笔记包含公开视频"
